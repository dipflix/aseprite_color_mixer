local utils_math = require('utils.math')
...
function M.colorShift(color, hueShift, satShift, lightShift, shadeShift)
    local newColor = Color(color)

    newColor.hue = newColor.hue + hueShift * 359

    if satShift > 0 then
        newColor.saturation = utils_math.lerp(newColor.saturation, 1, satShift)
    elseif satShift < 0 then
        newColor.saturation = utils_math.lerp(newColor.saturation, 0, -satShift)
    end

    if lightShift > 0 then
        newColor.lightness = utils_math.lerp(newColor.lightness, 1, lightShift)
    elseif lightShift < 0 then
        newColor.lightness = utils_math.lerp(newColor.lightness, 0, -lightShift)
    end
    ...
    if shadeShift >= 0 then
        newShade.hue = 50
        shadeInt = utils_math.lerpRGBInt(utils_math.colorToInt(newColor), utils_math.colorToInt(newShade), shadeShift)
    else
        newShade.hue = 215
        shadeInt = utils_math.lerpRGBInt(utils_math.colorToInt(newColor), utils_math.colorToInt(newShade), -shadeShift)
    end
...
function M.mix_linear_srgb(orig, dest, t)
    ...
    local r = utils_math.clamp(math.floor(0.5 + (mrLin ^ (1.0 / 2.2)) * 255.0), 0, 255)
    local g = utils_math.clamp(math.floor(0.5 + (mgLin ^ (1.0 / 2.2)) * 255.0), 0, 255)
    local b = utils_math.clamp(math.floor(0.5 + (mbLin ^ (1.0 / 2.2)) * 255.0), 0, 255)
    local a = utils_math.clamp(math.floor(0.5 + (u * orig.alpha + t * dest.alpha)), 0, 255)

    return Color(r, g, b, a)
end

function M.lab_to_ase_color(lab)
    ...
    local rClipped = utils_math.clamp(r255, 0, 255)
    local gClipped = utils_math.clamp(g255, 0, 255)
    local bClipped = utils_math.clamp(b255, 0, 255)

    return Color(rClipped, gClipped, bClipped, t)
end

function M.ase_color_to_lab(aseColor)
...
function M.mix_lab(orig, dest, t)
    local oLab = M.ase_color_to_lab(orig)
    local dLab = M.ase_color_to_lab(dest)
    local u = 1.0 - t

    local ml = u * oLab.l + t * dLab.l
    local ma = u * oLab.a + t * dLab.a
    local mb = u * oLab.b + t * dLab.b
    local mt = utils_math.clamp(math.floor(0.5 + (u * oLab.alpha + t * dLab.alpha)), 0, 255)

    return M.lab_to_ase_color({ l = ml, a = ma, b = mb, alpha = mt })
end

function M.lab_to_lch(lab)
...
function M.mix_lch(orig, dest, t, shortestHue)
    local oLab = M.ase_color_to_lab(orig)
    local dLab = M.ase_color_to_lab(dest)
    local o = M.lab_to_lch(oLab)
    local d = M.lab_to_lch(dLab)
    local u = 1.0 - t
    ...
    local l = u * o.l + t * d.l
    local c = u * o.c + t * d.c
    local a = utils_math.clamp(math.floor(0.5 + (u * o.alpha + t * d.alpha)), 0, 255)

    return M.lab_to_ase_color(M.lch_to_lab({ l = l, c = c, h = h, alpha = a }))
end

function M.srgb_to_oklab(color)
    local r = M.srgb_to_linear(color.red / 255.0)
    local g = M.srgb_to_linear(color.green / 255.0)
    local b = M.srgb_to_linear(color.blue / 255.0)
...
function M.oklab_to_srgb(ok)
    ...
    local rs = utils_math.clamp01(M.linear_to_srgb(r))
    local gs = utils_math.clamp01(M.linear_to_srgb(g))
    local bs = utils_math.clamp01(M.linear_to_srgb(b))

    return Color(
        utils_math.clamp(math.floor(0.5 + rs * 255.0), 0, 255),
        utils_math.clamp(math.floor(0.5 + gs * 255.0), 0, 255),
        utils_math.clamp(math.floor(0.5 + bs * 255.0), 0, 255),
        ok.alpha
    )
end

function M.oklab_to_oklch(ok)
...
function M.mix_oklab(orig, dest, t)
    local o = M.srgb_to_oklab(orig)
    local d = M.srgb_to_oklab(dest)
    local u = 1.0 - t
    ...
    return M.oklab_to_srgb({ l = L, a = A, b = B, alpha = a })
end

function M.mix_oklch(orig, dest, t, shortestHue)
    local o = M.oklab_to_oklch(M.srgb_to_oklab(orig))
    local d = M.oklab_to_oklch(M.srgb_to_oklab(dest))
    local u = 1.0 - t
    ...
    return M.oklab_to_srgb(M.oklch_to_oklab({ l = l, c = c, h = h, alpha = a }))
end
